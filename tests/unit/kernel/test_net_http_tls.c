...
void test_http_chunked_accumulator_fragmented(void){net_http_chunked_accumulator_t accumulator;net_http_response_view_t response;uint8_t buffer[128]={0};uint8_t first[]="HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\n\r\n4\r\nWi";uint8_t second[]="ki\r\n5\r\npedia\r\n0\r\n\r\n";uint8_t invalid[]="HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\n\r\nZ\r\n";TEST_ASSERT_EQUAL(0,net_http_chunked_accumulator_init(&accumulator,buffer,sizeof(buffer)));TEST_ASSERT_EQUAL(1,net_http_chunked_accumulator_feed(&accumulator,first,sizeof(first)-1U,&response));TEST_ASSERT_EQUAL(0,net_http_chunked_accumulator_feed(&accumulator,second,sizeof(second)-1U,&response));TEST_ASSERT_EQUAL(200,response.status_code);TEST_ASSERT_EQUAL(9,response.body_length);TEST_ASSERT_EQUAL_MEMORY("Wikipedia",response.body,9U);TEST_ASSERT_EQUAL(0,net_http_chunked_accumulator_init(&accumulator,buffer,sizeof(buffer)));TEST_ASSERT_NOT_EQUAL(0,net_http_chunked_accumulator_feed(&accumulator,invalid,sizeof(invalid)-1U,&response));}

void test_llm_json_string_extract(void){uint8_t output[32]={0};uint16_t length=0U;static const uint8_t json[]="{\"model\":\"gpt2\",\"response\":\"hello\\nworld\"}";static const uint8_t unicode[]="{\"response\":\"\\u0041\"}";TEST_ASSERT_EQUAL(0,net_json_extract_string(json,sizeof(json)-1U,"response",output,sizeof(output),&length));TEST_ASSERT_EQUAL(11,length);TEST_ASSERT_EQUAL_MEMORY("hello\nworld",output,11U);TEST_ASSERT_NOT_EQUAL(0,net_json_extract_string(json,sizeof(json)-1U,"missing",output,sizeof(output),&length));TEST_ASSERT_EQUAL(0,net_json_extract_string(unicode,sizeof(unicode)-1U,"response",output,sizeof(output),&length));TEST_ASSERT_EQUAL(1U,length);TEST_ASSERT_EQUAL('A',output[0]);TEST_ASSERT_NOT_EQUAL(0,net_json_extract_string(json,sizeof(json)-1U,"response",output,4U,&length));}

void test_llm_provider_response_adapters(void){uint8_t output[32]={0};uint16_t length=0U;static const uint8_t ollama[]="{\"model\":\"tinyllama\",\"response\":\"bonjour\",\"done\":true}";static const uint8_t openai[]="{\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"salut\"}}]}";TEST_ASSERT_EQUAL(0,net_llm_ollama_response_extract(ollama,sizeof(ollama)-1U,output,sizeof(output),&length));TEST_ASSERT_EQUAL(7,length);TEST_ASSERT_EQUAL_MEMORY("bonjour",output,7U);TEST_ASSERT_EQUAL(0,net_llm_openai_response_extract(openai,sizeof(openai)-1U,output,sizeof(output),&length));TEST_ASSERT_EQUAL(5,length);TEST_ASSERT_EQUAL_MEMORY("salut",output,5U);TEST_ASSERT_NOT_EQUAL(0,net_llm_ollama_response_extract(openai,sizeof(openai)-1U,output,sizeof(output),&length));TEST_ASSERT_NOT_EQUAL(0,net_llm_openai_response_extract(ollama,sizeof(ollama)-1U,output,sizeof(output),&length));}

void test_llm_request_json_builders(void){uint8_t output[192]={0};static const uint8_t prompt[]={'s','a','l','u','t',' ','"','x','"','\n'};static const uint8_t control[]={'a',1U};static const uint8_t ollama[]="{\"model\":\"tinyllama\",\"prompt\":\"salut \\\"x\\\"\\n\",\"stream\":false}";static const uint8_t openai[]="{\"model\":\"gpt-4o-mini\",\"messages\":[{\"role\":\"user\",\"content\":\"salut \\\"x\\\"\\n\"}],\"stream\":false}";TEST_ASSERT_EQUAL((int)(sizeof(ollama)-1U),net_llm_build_ollama_generate_json(output,sizeof(output),"tinyllama",prompt,sizeof(prompt)));TEST_ASSERT_EQUAL_MEMORY(ollama,output,sizeof(ollama)-1U);TEST_ASSERT_EQUAL((int)(sizeof(openai)-1U),net_llm_build_openai_chat_json(output,sizeof(output),"gpt-4o-mini",prompt,sizeof(prompt)));TEST_ASSERT_EQUAL_MEMORY(openai,output,sizeof(openai)-1U);TEST_ASSERT_NOT_EQUAL(0,net_llm_build_ollama_generate_json(output,sizeof(output),"tinyllama",control,sizeof(control)));TEST_ASSERT_NOT_EQUAL(0,net_llm_build_openai_chat_json(output,8U,"gpt-4o-mini",prompt,sizeof(prompt)));}

void test_llm_http_status_and_retry_policy(void){uint8_t retries=0U;TEST_ASSERT_EQUAL(NET_LLM_HTTP_STATUS_SUCCESS,net_llm_http_status_classify(200U));TEST_ASSERT_EQUAL(NET_LLM_HTTP_STATUS_SUCCESS,net_llm_http_status_classify(299U));TEST_ASSERT_EQUAL(NET_LLM_HTTP_STATUS_AUTH,net_llm_http_status_classify(401U));TEST_ASSERT_EQUAL(NET_LLM_HTTP_STATUS_AUTH,net_llm_http_status_classify(403U));TEST_ASSERT_EQUAL(NET_LLM_HTTP_STATUS_RETRYABLE,net_llm_http_status_classify(408U));TEST_ASSERT_EQUAL(NET_LLM_HTTP_STATUS_RETRYABLE,net_llm_http_status_classify(429U));TEST_ASSERT_EQUAL(NET_LLM_HTTP_STATUS_RETRYABLE,net_llm_http_status_classify(503U));TEST_ASSERT_EQUAL(NET_LLM_HTTP_STATUS_PERMANENT,net_llm_http_status_classify(400U));TEST_ASSERT_EQUAL(NET_LLM_HTTP_STATUS_PERMANENT,net_llm_http_status_classify(302U));TEST_ASSERT_EQUAL(NET_LLM_HTTP_STATUS_PROTOCOL,net_llm_http_status_classify(0U));TEST_ASSERT_EQUAL(1,net_llm_http_retry_consume(503U,2U,&retries));TEST_ASSERT_EQUAL(1U,retries);TEST_ASSERT_EQUAL(1,net_llm_http_retry_consume(429U,2U,&retries));TEST_ASSERT_EQUAL(2U,retries);TEST_ASSERT_EQUAL(0,net_llm_http_retry_consume(500U,2U,&retries));TEST_ASSERT_EQUAL(0,net_llm_http_retry_consume(401U,2U,&retries));TEST_ASSERT_EQUAL(2U,retries);TEST_ASSERT_NOT_EQUAL(0,net_llm_http_retry_consume(503U,1U,0));}
...
void test_llm_stream_json_and_sse_fragments(void){net_llm_sse_accumulator_t accumulator,tiny;uint8_t json[128]={0},buffer[96]={0},tiny_buffer[4]={0},text[32]={0};uint16_t text_length=0U;static const uint8_t ollama[]="{\"model\":\"tinyllama\",\"prompt\":\"hi\",\"stream\":true}";static const uint8_t openai[]="{\"model\":\"gpt-4o-mini\",\"messages\":[{\"role\":\"user\",\"content\":\"hi\"}],\"stream\":true}";TEST_ASSERT_EQUAL((int)(sizeof(ollama)-1U),net_llm_build_ollama_generate_stream_json(json,sizeof(json),"tinyllama",(const uint8_t*)"hi",2U));TEST_ASSERT_EQUAL_MEMORY(ollama,json,sizeof(ollama)-1U);TEST_ASSERT_EQUAL((int)(sizeof(openai)-1U),net_llm_build_openai_chat_stream_json(json,sizeof(json),"gpt-4o-mini",(const uint8_t*)"hi",2U));TEST_ASSERT_EQUAL_MEMORY(openai,json,sizeof(openai)-1U);TEST_ASSERT_EQUAL(0,net_llm_sse_accumulator_init(&accumulator,buffer,sizeof(buffer)));TEST_ASSERT_EQUAL(1,net_llm_sse_accumulator_feed(&accumulator,0U,(const uint8_t*)"data: {\"response\":\"bon",22U,text,sizeof(text),&text_length));TEST_ASSERT_EQUAL(0U,text_length);TEST_ASSERT_EQUAL(0,net_llm_sse_accumulator_feed(&accumulator,0U,(const uint8_t*)"jour\"}\n\ndata: [DONE]\n\n",22U,text,sizeof(text),&text_length));TEST_ASSERT_EQUAL(7U,text_length);TEST_ASSERT_EQUAL_MEMORY("bonjour",text,7U);TEST_ASSERT_EQUAL(1U,accumulator.done);TEST_ASSERT_EQUAL(0,net_llm_sse_accumulator_init(&tiny,tiny_buffer,sizeof(tiny_buffer)));TEST_ASSERT_NOT_EQUAL(0,net_llm_sse_accumulator_feed(&tiny,0U,(const uint8_t*)"data:",5U,text,sizeof(text),&text_length));TEST_ASSERT_NOT_EQUAL(0,net_llm_sse_accumulator_init(&accumulator,buffer,0U));TEST_ASSERT_EQUAL(0,net_llm_sse_accumulator_init(&accumulator,buffer,sizeof(buffer)));TEST_ASSERT_NOT_EQUAL(0,net_llm_sse_accumulator_feed(&accumulator,0U,(const uint8_t*)"event: bad\n\n",12U,text,sizeof(text),&text_length));}
void test_llm_sse_chunked_openai_response(void){net_llm_sse_response_t response;uint8_t http_buffer[160]={0},sse_buffer[96]={0},text[32]={0};uint16_t text_length=0U;static const uint8_t input[]="HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\n\r\n33\r\ndata: {\"choices\":[{\"delta\":{\"content\":\"salut\"}}]}\n\n\r\n0\r\n\r\n";TEST_ASSERT_EQUAL(0,net_llm_sse_response_init(&response,http_buffer,sizeof(http_buffer),sse_buffer,sizeof(sse_buffer)));TEST_ASSERT_EQUAL(0,net_llm_sse_response_feed(&response,1U,input,sizeof(input)-1U,text,sizeof(text),&text_length));TEST_ASSERT_EQUAL(5U,text_length);TEST_ASSERT_EQUAL_MEMORY("salut",text,5U);TEST_ASSERT_EQUAL(1U,response.http.state==8U);}

void test_llm_unicode_utf8_bounded(void){uint8_t output[96]={0},json[96]={0};uint16_t length=0U;static const uint8_t encoded[]="{\"response\":\"caf\\u00e9 \\ud83d\\ude00\"}";static const uint8_t expected[]={'c','a','f',0xc3U,0xa9U,' ',0xf0U,0x9fU,0x98U,0x80U};static const uint8_t prompt[]={'c','a','f',0xc3U,0xa9U};static const uint8_t invalid[]={'x',0xc3U};TEST_ASSERT_EQUAL(0,net_json_extract_string(encoded,sizeof(encoded)-1U,"response",output,sizeof(output),&length));TEST_ASSERT_EQUAL(sizeof(expected),length);TEST_ASSERT_EQUAL_MEMORY(expected,output,sizeof(expected));TEST_ASSERT_TRUE(net_llm_build_ollama_generate_json(json,sizeof(json),"tinyllama",prompt,sizeof(prompt))>0);TEST_ASSERT_NOT_EQUAL(0,net_llm_build_ollama_generate_json(json,sizeof(json),"tinyllama",invalid,sizeof(invalid)));}



void test_llm_large_fragmented_unicode_response(void){
    net_http_response_accumulator_t accumulator;net_http_response_view_t response;uint8_t wire[1200]={0},body[1024]={0},output[1024]={0};uint16_t body_length=1015U,first_length=300U,text_length=0U,index=0U;int status;
    static const uint8_t prefix[]={"{\"response\":\""};static const uint8_t suffix[]={'"','}'};static const uint8_t emoji[]={0xf0U,0x9fU,0x98U,0x80U};
    for(index=0U;index<sizeof(prefix)-1U;index++)body[index]=prefix[index];
    for(;index<sizeof(prefix)-1U+996U;index++)body[index]='a';
    for(index=0U;index<sizeof(emoji);index++)body[sizeof(prefix)-1U+996U+index]=emoji[index];
    body[sizeof(prefix)-1U+996U+sizeof(emoji)]=suffix[0];body[sizeof(prefix)-1U+996U+sizeof(emoji)+1U]=suffix[1];
    {static const uint8_t header[]={"HTTP/1.1 200 OK\r\nContent-Length: 1015\r\n\r\n"};for(index=0U;index<sizeof(header)-1U;index++)wire[index]=header[index];for(index=0U;index<body_length;index++)wire[(sizeof(header)-1U)+index]=body[index];}
    TEST_ASSERT_EQUAL(0,net_http_response_accumulator_init(&accumulator,wire,sizeof(wire)));status=net_http_response_accumulator_feed(&accumulator,wire,first_length,&response);TEST_ASSERT_EQUAL(1,status);status=net_http_response_accumulator_feed(&accumulator,wire+first_length,(uint16_t)((sizeof("HTTP/1.1 200 OK\r\nContent-Length: 1015\r\n\r\n")-1U)+body_length-first_length),&response);TEST_ASSERT_EQUAL(0,status);TEST_ASSERT_EQUAL(200,response.status_code);TEST_ASSERT_EQUAL(1000U,response.body_length-13U-2U);TEST_ASSERT_EQUAL(0,net_llm_ollama_response_extract(response.body,response.body_length,output,sizeof(output),&text_length));TEST_ASSERT_EQUAL(1000U,text_length);TEST_ASSERT_EQUAL('a',output[995]);TEST_ASSERT_EQUAL_MEMORY(emoji,output+996U,sizeof(emoji));
}


void test_llm_large_utf8_prompt_builder(void){
    uint8_t prompt[1020]={0},json[1152]={0};uint16_t index;int length;
    for(index=0U;index<1016U;index++)prompt[index]='a';
    prompt[1016U]=0xf0U;prompt[1017U]=0x9fU;prompt[1018U]=0x98U;prompt[1019U]=0x80U;
    length=net_llm_build_ollama_generate_json(json,sizeof(json),"tinyllama",prompt,sizeof(prompt));
