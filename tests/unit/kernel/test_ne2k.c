...
void test_ne2k_dhcp_acquire_guard_is_transactional(void){net_dhcp_lease_t lease={1U,{10U,0U,2U,15U},{10U,0U,2U,2U},0x12345678U};uint8_t tx[1]={0},rx[1]={0};TEST_ASSERT_NOT_EQUAL(0,ne2k_dhcp_acquire(0,0,tx,sizeof(tx),rx,sizeof(rx),0x12345678U,1U,&lease));TEST_ASSERT_EQUAL(1U,lease.valid);TEST_ASSERT_EQUAL(10U,lease.ipv4[0]);TEST_ASSERT_EQUAL(15U,lease.ipv4[3]);TEST_ASSERT_NOT_EQUAL(0,ne2k_dhcp_acquire(0,0,tx,sizeof(tx),rx,sizeof(rx),0x12345678U,0U,&lease));TEST_ASSERT_EQUAL(1U,lease.valid);TEST_ASSERT_NOT_EQUAL(0,ne2k_dhcp_poll_ack(0,0,rx,sizeof(rx),0x12345678U,1U,&lease));TEST_ASSERT_EQUAL(1U,lease.valid);}
