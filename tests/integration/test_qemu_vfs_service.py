...
def main():
    ...
    with open(ERR, "wb") as err_handle:
        ...
        try:
            ...
            send_command_until(monitor, "vfs-read vfs-mounts", "vfsserver virtual vfs-mounts", proc)
            ...
            wait_for("vfs-list ok count 2", proc, before_alias_list)
            wait_for("newdir", proc, before_alias_list)
            ...
            send_command_until(monitor, "vfs-list overlay/", "vfsserver list request", proc)
            wait_for("vfs-list ok count 3", proc, before_overlay_list)
            wait_for("newdir", proc, before_overlay_list)
