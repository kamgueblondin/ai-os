...
def main():
    ...
    with open(ERR, "wb") as err_handle:
        ...
        try:
            ...
            wait_for("vfs-list: montage absent", proc, before_missing_list)
            ...
            wait_for("request 13 data", proc, before_read)
            ...
            send_command(monitor, "vfs-stat initrd/hello.txt")
            wait_for("vfsserver stat request", proc, before_initrd_stat)
            ...
            send_command_until(monitor, "vfs-list overlay/", "vfsserver list request", proc)
            wait_for("vfs-list ok count 2", proc, before_overlay_list)
            wait_for("alias.txt", proc, before_overlay_list)
            wait_for("note.txt", proc, before_overlay_list)
            before_overlay_stat = len(log_text())
            send_command(monitor, "vfs-stat overlay/note.txt")
            wait_for("vfsserver stat request", proc, before_overlay_stat)
            ...
            send_command(monitor, "vfs-stat hello.txt")
            wait_for("vfsserver stat outside mounts", proc, before_stat_outside)
