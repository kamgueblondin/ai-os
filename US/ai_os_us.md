...
**Verification.** `make test-all` (**512/512**), `make qemu-vfs-service`, `make integration-qemu` (sept contrats séquentiels, **1 141,7 s** en validation locale, sans assertion retirée) puis `make qemu-ne2k-tls-multipair`. Le smoke cœur confirme chaque caractère PS/2 et efface localement un doublon avant `ret`, sans rejouer une ligne entrée.
...
|---|---|---|
| 0 | Budget CI QEMU | Conserver les sept contrats séquentiels sous 25 minutes et le smoke multi-pairs dans GitHub Actions, sans retirer d’assertion métier ni rejouer une mutation ou I/O incertaine ; la dernière mesure locale est 1 141,7 s |
