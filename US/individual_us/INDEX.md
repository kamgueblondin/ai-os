...
| US-008 | Framework de tests IA | Unity 299 + contrats QEMU (cœur, IRQ0, fournisseur IA, NE2000, IPC, saturation et état de service, VFS corrélé avec alias dynamiques, capacité, lectures, métadonnées et listage source-spécifiques de racine ou de sous-répertoire, écriture-suppression-renommage et statistiques, cycle de vie, transfert, révocation et notifications) + GitHub Actions ; pas de framework distribué |
| US-010 | Pilotes modulaires | PIC, PIT, PS/2, ATA PIO, NE2000 ISA ; pas de framework de drivers |
| US-012 | APIs unifiées | `include/os_syscalls.h` (syscalls 0–89), IPC avec `request_id` opaque, refus et état de capacité de service, événements noyau de service, VFS lecture-écriture-suppression-renommage et listage corrélés, gestion de montages par IPC, statistiques via source virtuelle, lectures et métadonnées backend source-spécifiques, registre avec retrait et transfert par propriétaire, FAT16 lecture seule, `SYS_NET_STATUS` |
