...
|---|---|---|
| US-001 | Microkernel + IPC | **Livraison partielle :** IPC, VFS Ring 3 lecture-écriture avec lectures initrd/overlay distinctes, découverte `vfs`, cycle de vie, corrélation, conservation différée, transfert, révocation, montages `initrd/ ro` et `overlay/ rw` bornés et notifications best-effort ; noyau monolithique, backend non externalisé |
...
| US-012 | APIs unifiées | `include/os_syscalls.h` (34 syscalls), IPC avec `request_id` opaque, événements noyau de service, VFS lecture-écriture corrélé et lectures backend source-spécifiques, registre avec retrait et transfert par propriétaire |
| US-013 | Communication inter-services | **Livraison partielle :** IPC, VFS local de lecture-écriture avec sources distinctes, registre, cycle de vie, corrélation, conservation bornée des réponses, transfert, révocation, montages statiques et notifications best-effort ; pas de capabilities, d’identité vérifiée ni de garantie de livraison |
