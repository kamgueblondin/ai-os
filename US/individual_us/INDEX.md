...
|---|---|---|
| US-001 | Microkernel + IPC | **Livraison partielle :** IPC, VFS Ring 3 lecture-écriture, découverte `vfs`, cycle de vie, corrélation, conservation différée, transfert, révocation, montages `initrd/ ro` et `overlay/ rw` bornés et notifications best-effort ; noyau monolithique, backend non externalisé |
...
| US-008 | Framework de tests IA | Unity 194 + contrats QEMU (cœur, IRQ0, fournisseur IA, IPC, VFS corrélé lecture-écriture, cycle de vie, transfert, révocation et notifications) + GitHub Actions ; pas de framework distribué |
| US-010 | Pilotes modulaires | PIC, PIT, PS/2, ATA PIO ; pas de framework de drivers |
| US-012 | APIs unifiées | `include/os_syscalls.h` (32 syscalls), IPC avec `request_id` opaque, événements noyau de service, VFS d’écriture corrélé et registre avec retrait et transfert par propriétaire |
| US-013 | Communication inter-services | **Livraison partielle :** IPC, VFS local de lecture-écriture, registre, cycle de vie, corrélation, conservation bornée des réponses, transfert, révocation, montages statiques et notifications best-effort ; pas de capabilities, d’identité vérifiée ni de garantie de livraison |
