...
- [ ] Planification périodique du renouvellement DHCP, handshake TLS HTTP complet, raccordement orchestral du client LLM à l’API socket et client OpenAI effectif
...
- [x] Tests complets du système corrigé (`make test-all` : 432 tests exécutés avec succès ; `make qemu-smoke` et `make integration-qemu`)
