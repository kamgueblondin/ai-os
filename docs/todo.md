...
- [ ] Planification périodique du renouvellement DHCP, handshake TLS actif et orchestration HTTP/SSE sur API socket, puis client OpenAI effectif
...
- [x] Tests complets du système corrigé (`make test-all` : 437 tests exécutés avec succès ; `make qemu-smoke` et `make integration-qemu`)
