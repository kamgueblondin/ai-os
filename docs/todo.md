...
- [x] Tests complets du système corrigé (`make test-all` : 487 tests exécutés avec succès ; `make qemu-smoke`, `make qemu-gguf-smoke`, `make integration-qemu`, `make qemu-vfs-service` et `make qemu-ne2k-acquire` jusqu’au ServerHello et son ACK sur pair Ethernet local contrôlé, et `make qemu-vfs-service` avec worker VFS Ring 3)
