...
Le vingt-deuxième incrément ajoute `SYS_VFS_INITRD_LISTDIR` (39) et `SYS_VFS_OVERLAY_LISTDIR` (40), réservés au propriétaire vivant de `vfs`. `vfs-list <prefixe/>` n’accepte qu’un montage déclaré et liste sa racine dans la source unique associée : aucun repli ni mélange initrd/overlay n’est effectué, y compris pour un alias dynamique. La requête porte le préfixe dans 48 octets ; la réponse corrélée de 80 octets porte un statut, au plus quatre noms séparés par `\n` dans 72 octets et un compte. Un statut `OS_VFS_STATUS_TRUNCATED` signifie qu’une page ne montre pas toutes les entrées ; il n’existe pas encore de curseur de pagination ni de listage de sous-répertoire médié.
...
make test-all                # 205/205 tests C Unity et robustesse
...
La suite C exécute 205 tests : PMM (17), syscall (48), tâches (21), overlay (8), tokenizer (15), GGUF (5), quantification (5), échantillonnage GPT-2 (4), IPC (6), file IPC différée (4), protocole VFS (20), registre de services (14), shell (25), RAMFS (10) et robustesse GGUF (3). `make integration-qemu` démarre six machines QEMU séparées : le contrat cœur AOS-022 utilise une image overlay de test isolée, le contrat IRQ0 AOS-024 préempte `spin`, le smoke AOS-025 vérifie que le profil OpenAI reste bloqué, le contrat Foundation livre un message à `ipcserver`, le contrat VFS résout `vfs`, installe et retire des alias dynamiques initrd/overlay, vérifie la capacité et la protection des montages initiaux, puis confirme lecture, métadonnées et listage source-spécifiques, écriture, suppression et renommage corrélés, sources virtuelles, compteurs, transfert, révocation et purge. Le contrat de transfert publie `demo`, observe une profondeur 0, remplit deux messages clients, observe une profondeur 2, exige le refus explicite du troisième, vide la file et observe une profondeur 0, puis abonne le shell, cède explicitement le CPU après le grant, vérifie la réaction événementielle de `serviceclaim`, puis l’événement de purge après `kill`. Les cadences du smoke cœur, extras, VFS et du contrat de service sont respectivement 0,65 s, 0,80 s, 0,90 s et 0,55 s pour limiter les doubles frappes PS/2 sous QEMU TCG ; les contrats VFS et service relancent au plus trois commandes longues jusqu’à leur marqueur fonctionnel, sans assouplir les assertions.
