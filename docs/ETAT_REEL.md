...
L’ABI contient les syscalls 0–43 (`MAX_SYSCALLS = 44`), dont `SYS_APPEND`, `SYS_GPT2_GENERATE`, `SYS_IPC_SEND`, `SYS_IPC_RECV`, `SYS_SERVICE_REGISTER`, `SYS_SERVICE_LOOKUP`, `SYS_SERVICE_UNREGISTER`, `SYS_SERVICE_GRANT`, `SYS_VFS_BACKEND_READ`, `SYS_SERVICE_NOTIFY`, `SYS_VFS_BACKEND_WRITE`, `SYS_VFS_INITRD_READ`, `SYS_VFS_OVERLAY_READ`, `SYS_VFS_OVERLAY_UNLINK`, `SYS_VFS_OVERLAY_RENAME`, `SYS_SERVICE_STATUS`, `SYS_VFS_INITRD_STAT`, `SYS_VFS_OVERLAY_STAT`, `SYS_VFS_INITRD_LISTDIR` et `SYS_VFS_OVERLAY_LISTDIR`.
...
make test-all                # 209/209 tests C Unity et robustesse
...
La suite C exécute 209 tests : PMM (17), syscall (48), tâches (21), overlay (8), tokenizer (15), GGUF (5), quantification (5), échantillonnage GPT-2 (4), IPC (6), file IPC différée (4), protocole VFS (24), registre de services (14), shell (25), RAMFS (10) et robustesse GGUF (3). `make integration-qemu` démarre six machines QEMU séparées : le contrat cœur AOS-022 utilise une image overlay de test isolée, le contrat IRQ0 AOS-024 préempte `spin`, le smoke AOS-025 vérifie que le profil OpenAI reste bloqué, le contrat Foundation livre un message à `ipcserver`, le contrat VFS résout `vfs`, installe et retire des alias dynamiques initrd/overlay, vérifie la capacité et la protection des montages initiaux, puis confirme lecture, métadonnées et listage source-spécifiques de racine et de sous-répertoire, écriture, suppression et renommage corrélés, sources virtuelles, compteurs, transfert, révocation et purge. Le contrat de transfert publie `demo`, observe une profondeur 0, remplit deux messages clients, observe une profondeur 2, exige le refus explicite du troisième, vide la file et observe une profondeur 0, puis abonne le shell, cède explicitement le CPU après le grant, vérifie la réaction événementielle de `serviceclaim`, puis l’événement de purge après `kill`. Les cadences du smoke cœur, extras, VFS et du contrat de service sont respectivement 0,65 s, 0,80 s, 0,90 s et 0,55 s pour limiter les doubles frappes PS/2 sous QEMU TCG ; les contrats VFS et service relancent au plus trois commandes longues jusqu’à leur marqueur fonctionnel, sans assouplir les assertions.
