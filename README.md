...
| Utilisateur | Shell ELF Ring 3, syscalls 0–40, `spawn`, `yield`, `exec`, `ps`, `kill` |
...
| VFS Foundation | `vfsserver` Ring 3, sources `vfs-info`/`vfs-mounts`/`vfs-stats`, deux montages protégés et trois alias dynamiques initrd/overlay au plus, lectures, métadonnées et listage source-spécifiques, écriture, suppression et renommage médiés, compteurs volatils, transfert contrôlé de `vfs` et backend réservé au propriétaire publié courant ; initrd/overlay encore noyau |
...
| `make test-all` | 205 tests C Unity/robustesse sans dépendre des poids GPT-2 |
...
| `make qemu-vfs-service` | Lance `vfsserver`, vérifie listage source-spécifique, alias dynamiques initrd/overlay, capacité, refus, mutations corrélées, transfert et révocation |
...
`make test-all` a validé **205/205** tests : PMM (17), syscall (48), tâches (21), overlay (8), tokenizer (15), GGUF (5), quantification (5), échantillonnage GPT-2 (4), IPC (6), file IPC différée (4), protocole VFS (20), registre de services (14), shell (25), RAMFS (10) et robustesse GGUF (3). `make integration-qemu` ajoute six validations QEMU séparées, dont les contrats IPC, capacité et instantanés de profondeur d’un propriétaire de service publié, médiateur VFS corrélé avec conservation locale, alias dynamiques initrd/overlay, capacité et protection de la table, métadonnées et listage source-spécifiques, écriture, suppression et renommage médiés, sources virtuelles, compteurs VFS, transfert, révocation et notifications de service ; il réinitialise son disque de contrat sans toucher à `build/overlay.img`. Les délais de frappe QEMU sont stabilisés à 0,65 s pour le smoke cœur, 0,80 s pour les extras, 0,90 s pour le contrat VFS et 0,55 s pour le contrat de service ; ces contrats relancent au plus trois commandes longues jusqu’à leur marqueur fonctionnel attendu.
...
- [x] Listage VFS médié : page de quatre noms au plus par source déclarée via `vfs-list <prefixe/>`
