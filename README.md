...
| VFS Foundation | `vfsserver` Ring 3, sources `vfs-info`/`vfs-mounts`/`vfs-stats`, quatre montages protégés (`initrd/`, `overlay/`, `fat16/`, `fat32/`) et quatre alias dynamiques au plus, lectures, métadonnées et listage source-spécifiques ; FAT16/FAT32 admettent fichiers 8.3 dans un sous-répertoire 8.3 unique et LFN conservés à la racine, avec création, suppression, renommage, `mkdir`/`rmdir` vide, sans écrasement ni rejeu après mutation incertaine ; pas de niveau imbriqué, de LFN enfant, de renommage inter-répertoire ni de remplacement atomique ; compteurs volatils, transfert contrôlé de `vfs`, délégation backend révocable avec profils `read`/`mutate`/`full`, consultation unitaire et inventaire borné réservés au propriétaire publié courant |
...
| `make test-all` | Suite complète Unity/robustesse ; l’état validé courant est de 505 tests exécutés avec succès |
...
| `make qemu-vfs-service` | Lance `vfsvirtual` puis `vfsserver`, vérifie la délégation Ring 3, deux volumes IDE FAT16/FAT32, les capacités et refus, les mutations overlay et les cycles FAT racine/LFN ainsi que `mkdir`, écriture, `stat`, liste, renommage, refus `rmdir` non vide, suppression et `rmdir` d’un sous-répertoire 8.3 |
