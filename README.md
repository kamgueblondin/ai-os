...
| Utilisateur | Shell ELF Ring 3, ABI de syscalls 0-121 (`MAX_SYSCALLS = 122`), `spawn`, `yield`, `exec`, `ps`, `kill` |
...
| Fichiers | Archive initrd TAR en lecture seule, overlay AIOV V2 sur ATA PIO (64 nœuds, V1 compatible), transferts sectoriels x86 `rep insw`/`rep outsw`, volumes FAT16 et FAT32 accessibles en lecture, listage et statut via le VFS ; le médiateur `vfsserver` délègue `vfs-info`, le formatage de `vfs-stats`, les lignes de `vfs-mounts` et leurs pages `vfs-list-page`/`vfs-list-observe` à `vfsvirtual` Ring 3 sur IPC borné, avec repli local, récupération corrélée si le worker disparaît pendant une transaction privée, vue locale `vfs-worker` de PID/absence, compteurs de récupérations et d’expirations après huit tours sans réponse, puis redécouverte après relance du worker, tandis que les mutations VFS restent limitées à l’overlay, à l’exception de la création, de la suppression et du renommage FAT16 8.3 à la racine sous capacité `mutate` |
...
| Réseau | Pilote NE2000 ISA, `SYS_NET_STATUS`, codecs ARP/IPv4/UDP/DHCP/DNS/TCP/TLS record, renouvellement, réacquisition et backoff DHCP caller-owned différés hors IRQ0, conservation fournisseur et reprise HTTP/SSE contrôlées, reprise SSE fine `Last-Event-ID`, registre TCP statique et orchestrateur noyau transactionnel. `ai-acquire` puis `ai-tls-poll` valident DHCP/OFFER/REQUEST/ACK, ARP, DNS A, SYN, SYN-ACK, ClientHello et ServerHello minimal sur un pair Ethernet QEMU local. `make qemu-ne2k-tls-http` ajoute le TLS 1.2 authentifie local et un POST HTTP 200 JSON ; `make qemu-ne2k-tls-sse` prolonge ce pair par un flux SSE chunked (`SSE : ok` puis cloture `[DONE]`). OpenAI externe et Internet public restent hors livraison. |
...
| `make test-all` | Suite complete Unity/robustesse ; l'etat valide est de 496 tests executes avec succes |
