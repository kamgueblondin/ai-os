...
| Utilisateur | Shell ELF Ring 3, ABI de syscalls 0-123 (`MAX_SYSCALLS = 124`), `spawn`, `yield`, `exec`, `ps`, `kill` |
...
| VFS Foundation | `vfsserver` Ring 3, sources `vfs-info`/`vfs-mounts`/`vfs-stats`, quatre montages protégés (`initrd/`, `overlay/`, `fat16/`, `fat32/`) et quatre alias dynamiques au plus. `vfsvirtual` Ring 3 valide et stocke les ajouts/retraits d’alias sous messages corrélés ; `vfsserver` ne conserve qu’un miroir de routage I/O volatile, modifié après réponse et purgé à tout changement, disparition ou expiration incertaine du worker. Le worker ne reçoit pas le droit backend `read`, ses mutations privées sont limitées au médiateur `vfs` publié, et aucune mutation incertaine n’est rejouée. FAT16/FAT32 admettent fichiers 8.3 dans un sous-répertoire 8.3 unique et LFN conservés à la racine, avec création, suppression, renommage, `mkdir`/`rmdir` vide, sans écrasement ; pas de niveau imbriqué, de LFN enfant, de renommage inter-répertoire ni de remplacement atomique ; compteurs volatils, transfert contrôlé de `vfs`, délégation backend révocable avec profils `read`/`mutate`/`full`, consultation unitaire et inventaire borné réservés au propriétaire publié courant |
...
| Fichiers | Archive initrd TAR en lecture seule, overlay AIOV V2 sur ATA PIO (64 nœuds, V1 compatible), transferts sectoriels x86 `rep insw`/`rep outsw`, volumes FAT16 et FAT32 accessibles en lecture, listage, pagination et statut via le VFS ; `vfsserver` délègue à `vfsvirtual` Ring 3 sous le seul droit `mutate` les mutations de montages fixes, y compris `mkdir`/`rmdir` FAT. Les ajouts et retraits d’alias sont validés et mémorisés par le même worker ; le médiateur applique seulement un miroir de routage après réponse corrélée, puis le purge et retourne un échec corrélé sans rejeu si le worker est remplacé, absent ou expire après l’envoi. FAT conserve les LFN racine et publie un seul niveau enfant 8.3, sans écrasement, niveau imbriqué, LFN enfant, renommage inter-répertoire ou remplacement atomique |
...
| `make test-all` | Suite complète Unity/robustesse ; l’état validé courant est de 506 tests exécutés avec succès |
...
| `make integration-qemu` | Sept contrats QEMU AOS-022, AOS-024, AOS-025, NE2000, IPC, VFS avec montages dynamiques, mutations médiées, révocation, notifications, cycle de vie et transfert Foundation ; validation locale mesurée à 417,1 s sans assertion retirée |
...
- [~] Migration microkernel réelle : `vfsserver` délègue à `vfsvirtual` Ring 3 les vues publiques et les mutations des montages fixes `overlay/`, `fat16/` et `fat32/`, y compris les répertoires FAT, sous capacité `mutate` minimale et corrélation PID/requête ; une mutation en vol devenue incertaine échoue sans rejeu automatique ; les alias dynamiques et la séparation complète du pilote de stockage restent ouverts
...
- [x] Ramener `make integration-qemu` sous 25 minutes sans relâcher les assertions métier : sept contrats complets en 23 min 30 s en validation locale
...
- [ ] Déléguer au worker Ring 3 les opérations VFS encore locales (alias dynamiques et répertoires) avec droits minimaux et corrélation conservée
...
- [ ] Contrat TAP/bridge ou multi-pairs QEMU strictement local, sans clé ni hôte Internet, afin d’élargir la topologie contrôlée
