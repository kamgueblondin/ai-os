...
| Fichiers | Archive initrd TAR en lecture seule, overlay AIOV V2 sur ATA PIO (64 nœuds, V1 compatible), transferts sectoriels x86 `rep insw`/`rep outsw`, volumes FAT16 et FAT32 accessibles en lecture, listage, pagination et statut via le VFS ; `vfsserver` délègue à `vfsvirtual` Ring 3 les mutations fixes sous `mutate` et les I/O d’alias sous `read` transitoire. Le worker ne route que les alias qu’il détient ; le médiateur applique uniquement un miroir de sélection après réponse corrélée. Après remplacement, disparition ou expiration, le miroir est purgé et la transaction retourne `INVALID` sans rejeu ; le droit antérieur est restauré ou la capacité est retirée. FAT conserve les LFN racine et publie un seul niveau enfant 8.3, sans écrasement, niveau imbriqué, LFN enfant, renommage inter-répertoire ou remplacement atomique |
...
| `make test-all` | Suite complète Unity/robustesse ; l’état validé courant est de 509 tests exécutés avec succès |
...
| `make integration-qemu` | Sept contrats QEMU AOS-022, AOS-024, AOS-025, NE2000, IPC, VFS avec montages dynamiques, I/O d’alias médiées, révocation, notifications, cycle de vie et transfert Foundation ; l’ordonnanceur est séquentiel par défaut pour éviter la contention PS/2 et a validé les sept contrats en 786,7 s, sans assertion retirée |
