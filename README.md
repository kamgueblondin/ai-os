...
| Utilisateur | Shell ELF Ring 3, syscalls 0–34, `spawn`, `yield`, `exec`, `ps`, `kill` |
...
| VFS Foundation | `vfsserver` Ring 3, sources `vfs-info`/`vfs-mounts`, montages `initrd/ ro` et `overlay/ rw`, lectures source-spécifiques, écriture et suppression médiées, transfert contrôlé de `vfs` et backend réservé au propriétaire publié courant ; initrd/overlay encore noyau |
...
Les commandes du shell comprennent notamment `ls`, `cat`, `mkdir`, `rmdir`, `rm`, `cp`, `mv`, `write`, `append`, `touch`, `stat`, `grep`, `wc`, `spawn`, `yield`, `ipc-send`, `ipc-recv`, `service-publish`, `service-grant`, `service-find`, `service-watch`, `vfs-backend-probe <fichier>`, `vfs-backend-write-probe <fichier> <texte>`, `vfs-backend-remove-probe <fichier>`, `vfs-grant <pid>`, `vfs-read <chemin>`, `vfs-write <chemin> <texte>`, `vfs-remove <chemin>`, `jobs`, `top`, `ai`, `ai-provider`, `ai-model`, `ai-runtime` et `net-status`.
 `service-watch <nom>` abonne le shell à un service et `ipc-recv` affiche les transitions avec l’ancien PID, le nouveau PID et la raison ; la livraison est best-effort si la boîte IPC est pleine. `vfs-read` résout le service `vfs` au lieu d’accepter un PID ; le médiateur expose `vfs-read vfs-mounts`, sert `initrd/` depuis l’archive initrd exclusivement et `overlay/` depuis l’overlay ATA exclusivement, puis autorise `vfs-write overlay/note.txt texte` et `vfs-remove overlay/note.txt` sur le seul montage modifiable. Les chemins globaux, les écritures et les suppressions sous `initrd/` sont refusés par la politique de montage ; une requête d’écriture est bornée à 44 octets.
...
| `make test-all` | 196 tests C Unity/robustesse sans dépendre des poids GPT-2 |
| `make qemu-smoke` | Scénarios QEMU classiques : overlay, persistance, spawn/yield et exec |
| `make integration-qemu` | Contrats QEMU AOS-022, AOS-024, AOS-025, IPC, VFS lecture-écriture-suppression médié, révocation, notifications, cycle de vie et transfert Foundation |
...
| `make qemu-vfs-service` | Lance `vfsserver`, vérifie `initrd/ ro`, `overlay/ rw`, les refus, la lecture-écriture-suppression corrélée, le transfert et la révocation |
...
`make test-all` a validé **196/196** tests : PMM (17), syscall (48), tâches (21), overlay (8), tokenizer (15), GGUF (5), quantification (5), échantillonnage GPT-2 (4), IPC (6), file IPC différée (4), protocole VFS (12), registre de services (13), shell (25), RAMFS (10) et robustesse GGUF (3). `make integration-qemu` ajoute six validations QEMU séparées, dont les contrats IPC, médiateur VFS corrélé avec conservation locale, montages `initrd/ ro` et `overlay/ rw` lus par sources distinctes, écriture et suppression médiées, sources virtuelles, transfert, révocation et notifications de service ; il réinitialise son disque de contrat sans toucher à `build/overlay.img`.
