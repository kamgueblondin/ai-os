...
| Réseau | Pilote NE2000 ISA, `SYS_NET_STATUS`, codecs ARP/IPv4/UDP/DHCP/DNS/TCP/TLS record, renouvellement DHCP live caller-owned, registre TCP statique, SYN actif, ClientHello TLS caller-owned et construction/réception HTTP/SSE LLM sur socket, ponts TCP NE2000 RX/TX ; planification DHCP, polling TLS authentifié et orchestrateur OpenAI restent à intégrer |
...
| `make test-all` | Suite complète Unity/robustesse ; l’état courant validé est de 438 tests exécutés avec succès |
