...
| Réseau | Pilote NE2000 ISA, `SYS_NET_STATUS`, codecs ARP/IPv4/UDP/DHCP/DNS/TCP/TLS record, registre TCP statique exposé par six syscalls socket et primitives TLS caller-owned `send_tls`/`receive_tls` ; orchestration HTTP/TLS LLM et OpenAI restent à intégrer |
...
| `make test-all` | Suite complète Unity/robustesse ; l’état courant validé est de 428 tests exécutés avec succès |
