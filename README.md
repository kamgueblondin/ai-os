...
- [~] Migration microkernel réelle : première délégation `vfsserver` → `vfsvirtual` Ring 3 livrée et résiliente avant soumission pour `vfs-info`, `vfs-stats` et `vfs-mounts` ; backend de stockage processus séparé et transferts de données hors du noyau encore ouverts
