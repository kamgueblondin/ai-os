...
| Fichiers | Archive initrd TAR en lecture seule, overlay AIOV V2 sur ATA PIO (64 nœuds, V1 compatible) et volume FAT16 lecture seule à partir du LBA 64 |
...
| Réseau | Pilote NE2000 ISA, `SYS_NET_STATUS`, codecs ARP/IPv4/UDP/DHCP/DNS/TCP et TLS record ; OpenAI toujours bloqué |
...
| `make test-all` | 299 tests C Unity/robustesse sans dépendre des poids GPT-2 |
...
Le profil `ai-provider openai` est un **stub contrôlé**. `net-status` / `net-status json` publient la présence réelle d’une NIC NE2000 ISA (smoke `make qemu-ne2k-status`) ; ARP, IPv4, DHCP, DNS, TCP et TLS restent affichés absents tant qu’aucune configuration live n’est raccordée. Une commande `ai` avec le profil OpenAI ne transmet aucune requête. QEMU peut connecter `ne2k_isa` à un backend hôte ; le guest possède désormais un pilote et des codecs caller-owned, pas un client HTTP [1]. Les secrets OpenAI ne doivent jamais être inclus dans l’image, les logs série ou le dépôt.

## Tests et artefacts

`make test-all` a validé **299/299** tests (17 août 2026), dont FAT16, console VGA, PCI, SHA-256/HMAC, codecs Ethernet/ARP/IPv4/UDP/DHCP/DNS/TCP, files NIC, NE2000 et TLS record, en plus des suites PMM, syscall, tâches, overlay, tokenizer, GGUF, IPC, VFS, services, shell et RAMFS. `make integration-qemu` ajoute sept validations QEMU séparées, dont le smoke NE2000 et les contrats IPC, VFS et service ; il réinitialise son disque de contrat sans toucher à `build/overlay.img`. Les délais de frappe QEMU sont stabilisés à 0,65 s pour le smoke cœur, 0,80 s pour les extras, 1,10 s pour le contrat VFS et 0,55 s pour le contrat de service ; ces contrats relancent au plus trois commandes jusqu’à leur marqueur fonctionnel attendu, notamment pour les chemins IPC, VFS et service sensibles aux doubles frappes PS/2.
...
- [ ] Écriture FAT, LFN et FAT32 (pas ext2) — [docs/aos_fat_volume.md](docs/aos_fat_volume.md)
