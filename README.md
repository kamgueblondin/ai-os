...
| VFS Foundation | `vfsserver` Ring 3, sources `vfs-info`/`vfs-mounts`/`vfs-stats`, quatre montages protégés (`initrd/`, `overlay/`, `fat16/`, `fat32/`) et quatre alias dynamiques au plus, lectures, métadonnées et listage source-spécifiques ; écriture, suppression et renommage pour l’overlay ainsi que pour les fichiers FAT16/FAT32 8.3 ou LFN à la racine sous `mutate`, sans écrasement, sous-répertoire VFS ni remplacement atomique ; compteurs volatils, transfert contrôlé de `vfs`, délégation backend révocable avec profils `read`/`mutate`/`full`, consultation unitaire et inventaire borné réservés au propriétaire publié courant |
...
| `make test-all` | Suite complète Unity/robustesse ; l’état validé courant est de 502 tests exécutés avec succès |
...
- [ ] Ramener durablement `make integration-qemu` sous 25 minutes, sans relâcher les assertions métier, puis confirmer par plusieurs passages locaux et CI
- [ ] Étendre le VFS aux sous-répertoires FAT sous un contrat de non-écrasement et de non-rejeu des mutations incertaines
