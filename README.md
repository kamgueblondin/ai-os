...
| Réseau | Pilote NE2000 ISA, `SYS_NET_STATUS`, codecs ARP/IPv4/UDP/DHCP/DNS/TCP/TLS record, renouvellement DHCP live caller-owned, registre TCP statique, TLS caller-owned, SYN actif et construction/réception HTTP/SSE LLM sur socket, ponts TCP NE2000 RX/TX ; planification DHCP, handshake TLS actif et orchestrateur OpenAI restent à intégrer |
...
| `make test-all` | Suite complète Unity/robustesse ; l’état courant validé est de 437 tests exécutés avec succès |
