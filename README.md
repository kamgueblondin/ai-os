...
| Réseau | Pilote NE2000 ISA, `SYS_NET_STATUS`, codecs ARP/IPv4/UDP/DHCP/DNS/TCP/TLS record, registre TCP statique, TLS caller-owned `send_tls`/`receive_tls`, construction LLM Ollama/OpenAI sur socket et pont de segment vers TX NE2000 ; polling HTTP/SSE socket et orchestration OpenAI complète restent à intégrer |
...
| `make test-all` | Suite complète Unity/robustesse ; l’état courant validé est de 431 tests exécutés avec succès |
