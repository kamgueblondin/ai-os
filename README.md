...
| Fichiers | Archive initrd TAR en lecture seule, overlay AIOV V2 sur ATA PIO (64 nœuds, V1 compatible), transferts sectoriels x86 `rep insw`/`rep outsw`, volumes FAT16 et FAT32 accessibles en lecture, listage et statut via le VFS ; les mutations VFS restent limitées à l’overlay |
...
| `make qemu-ne2k-acquire` | Valide `ai-acquire example.com` via DHCP, ARP, DNS A et SYN contre un pair Ethernet socket local déterministe |
| `make qemu-ipc-foundation` | Lance `ipcserver`, envoie un message et vérifie sa réception |
| `make qemu-vfs-service` | Lance `vfsserver`, vérifie les volumes FAT16/FAT32 réels sur deux disques IDE, les alias dynamiques, la capacité, les refus, les mutations overlay, les profils backend, le transfert et la révocation |
...
- [ ] Migration microkernel réelle : backend processus séparé et transferts de données hors du noyau
...
- [x] Bootstrap DHCP/OFFER/REQUEST/ACK, ARP, DNS A et SYN LLM observé sur NE2000 QEMU avec pair Ethernet local contrôlé
- [ ] Validation sur réseau physique/TAP ou bridge réel, SYN-ACK, handshake TLS, HTTP et client OpenAI effectif
