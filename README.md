...
| VFS Foundation | `vfsserver` Ring 3, sources `vfs-info`/`vfs-mounts`/`vfs-stats`, quatre montages protégés (`initrd/`, `overlay/`, `fat16/`, `fat32/`) et quatre alias dynamiques au plus, lectures, métadonnées et listage source-spécifiques ; écriture, suppression et renommage complets pour l’overlay, création et suppression de fichiers FAT16 8.3 racine sous `mutate`, compteurs volatils, transfert contrôlé de `vfs`, délégation backend révocable avec profils `read`/`mutate`/`full`, consultation unitaire et inventaire borné réservés au propriétaire publié courant |
...
| `make test-all` | Suite complète Unity/robustesse ; l’état validé est de 488 tests exécutés avec succès |
...
| `make qemu-vfs-service` | Lance `vfsvirtual` puis `vfsserver`, vérifie la délégation Ring 3, les volumes FAT16/FAT32 réels sur deux disques IDE, les alias dynamiques, les capacités, les refus, les mutations overlay et le cycle FAT16 création/lecture/listage/suppression sous `mutate` |
