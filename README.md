...
| Réseau | Pilote NE2000 ISA, `SYS_NET_STATUS`, codecs ARP/IPv4/UDP/DHCP/DNS/TCP/TLS record, renouvellement DHCP live caller-owned, registre TCP statique, TLS caller-owned, construction et réception HTTP/SSE LLM sur socket, pont de segment vers TX NE2000 ; planification DHCP et orchestrateur OpenAI complet restent à intégrer |
...
| `make test-all` | Suite complète Unity/robustesse ; l’état courant validé est de 434 tests exécutés avec succès |
