...
| Réseau | Pilote NE2000 ISA, `SYS_NET_STATUS`, codecs ARP/IPv4/UDP/DHCP/DNS/TCP/TLS record, renouvellement DHCP live caller-owned, registre TCP statique, SYN actif, ClientHello et polling TLS authentifié caller-owned (X.509, X25519, post-flight), session LLM socket de SYN-ACK à HTTP/SSE via NE2000 ; planification DHCP, bootstrap DHCP/DNS/ARP/SYN vers cette session et client OpenAI effectif restent à intégrer |
...
| `make test-all` | Suite complète Unity/robustesse ; l’état courant validé est de 442 tests exécutés avec succès |
...
`make test-all` a validé **442 tests exécutés avec succès** dans l’état courant, dont FAT16/FAT32, console VGA, PCI, SHA-256/HMAC, codecs Ethernet/ARP/IPv4/UDP/DHCP/DNS/TCP, NE2000, TLS authentifié et session LLM HTTP/SSE sur socket, GPT-2/GGUF, IPC, VFS, services, shell et RAMFS. `make integration-qemu` ajoute les validations QEMU séparées, dont le smoke NE2000 et les contrats IPC, VFS et service ; il réinitialise son disque de contrat sans toucher à `build/overlay.img`. Les détails de périmètre et les limites restantes sont maintenus dans [docs/ETAT_REEL.md](docs/ETAT_REEL.md) et [docs/todo.md](docs/todo.md).
