...
| `make qemu-ne2k-acquire` | Valide `ai-acquire example.com` puis `ai-tls-poll` via DHCP, ARP, DNS A, SYN/SYN-ACK et ClientHello contre un pair Ethernet socket local déterministe |
...
- [x] Bootstrap DHCP/OFFER/REQUEST/ACK, ARP, DNS A, SYN/SYN-ACK et ClientHello LLM observé sur NE2000 QEMU avec pair Ethernet local contrôlé
