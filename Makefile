...
run: $(OS_IMAGE) pack-initrd
	qemu-system-i386 -kernel $(OS_IMAGE) -initrd $(INITRD_IMAGE) -nographic -serial file:serial.log -d int,guest_errors,cpu_reset -no-reboot -no-shutdown -monitor telnet:localhost:4444,server,nowait
