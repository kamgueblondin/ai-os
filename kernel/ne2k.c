...
int ne2k_llm_network_context_dhcp_renew_if_due(ne2k_llm_network_context_t* context,ne2k_device_t* device,const ne2k_io_t* io,uint8_t* tx_frame,uint16_t tx_capacity,uint8_t* rx_frame,uint16_t rx_capacity,uint32_t xid,uint16_t poll_attempts,uint32_t now){net_dhcp_lease_t next_lease;int status;if(!context)return -1;next_lease=context->lease;status=ne2k_dhcp_renew_if_due(device,io,tx_frame,tx_capacity,rx_frame,rx_capacity,xid,poll_attempts,now,&next_lease);if(status<0)return status;if(status==1)context->lease=next_lease;return status;}
...
int ne2k_llm_socket_session_poll_tls(ne2k_device_t* device,const ne2k_io_t* io,const net_arp_cache_t* cache,
    ...
    ne2k_llm_socket_session_t previous_session; ne2k_tls_client_t previous_client; int status;
    if (!session || !client) return -1;
